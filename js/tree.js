...
Node.prototype.draw = function (o) {
  if (!this.is_cutted() && this.w > 0 && this.h > 0) {
    ...
    r.linewidth = 2;
    two.makeText("" + this.w + "x" + this.h, o.x, o.y, { size: 20 });
    //   two.makeText("" + this.id, o.x, o.y + 40, { size: 20 })

    if (this.is_piece) r.fill = "#6DCC6E";
    else r.fill = "#CCC272";
  ...
  if (this.is_cutted()) {
    if (this.cutside == "w") {
      ...
      this.right.draw({
        x: o.x - this.w / 2 + this.left.w + this.right.w / 2,
        y: o.y,
      });
    } else {
      // this.cutside == 'h'
      this.left.draw({
        x: o.x,
        y: o.y - this.h / 2 + this.right.h + this.left.h / 2,
      });
...
Node.try_push = function (piece) {
  for (node in Node.pieces) {
    node = Node.pieces[node];
    if (
      !node.is_piece &&
      !node.is_cutted() &&
      node.w >= piece.w &&
